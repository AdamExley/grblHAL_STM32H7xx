...
static inline void setTX ()
{
    ...
    DIGITAL_OUT(active_uart->port, 1 << active_uart->pin, 1);
}
...
{
    if(tx_buf.data) {
        DIGITAL_OUT(active_uart->port, 1 << active_uart->pin, tx_buf.data & 1);  // drive bit out
...
static void rcv (void)
{
    ...
    bool inbit = DIGITAL_IN(active_uart->port, 1 << active_uart->pin);
...
TMC_uart_write_datagram_t *tmc_uart_read (trinamic_motor_t driver, TMC_uart_read_datagram_t *rdgr)
{
    ...
    while (DIGITAL_IN(active_uart->port, 1 << active_uart->pin)) {
...
{
//    // debug: toggle PA1 in soft UART IRQ
//    static GPIO_InitTypeDef GPIO_InitStruct = {
//        .Mode = GPIO_MODE_OUTPUT_PP,
//        .Pull = GPIO_NOPULL
//    };

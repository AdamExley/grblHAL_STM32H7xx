...
__attribute__((section(".itcmram"))) static void ISR_FUNC(i2c_enqueue_keycode)(char c)
{
...
__attribute__((section(".itcmram"))) bool ISR_FUNC(keypad_strobe_handler)(uint_fast8_t id, bool keydown)
{
    keyreleased = !keydown;
...
static __attribute__((section(".itcmram"))) bool ISR_FUNC(keypad_enqueue_keycode)(uint8_t c)
{
    uint32_t bptr = (keybuf.head + 1) & (KEYBUF_SIZE - 1);    // Get next head pointer
